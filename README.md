...
скачиваются локально; реконструированные ключи к мокам живут только локально
в `klucze/` (см. `docs/DIAGNOSTYKA.md`):
...
мог бы присвоить первый нажавший `/start`).
...
`webapp/` — полностью статическое приложение в нативном стиле Telegram: хаб плашек,
те же тренажёры, что в боте (грамматика, аудио-задание I, времена, трансформации),
счётчик объёма письма, таймер устной части с диалогом задания 3, план и статистика
(localStorage). Деплой — GitHub Pages (`.github/workflows/pages.yml`), после первого
деплоя вписать адрес в `WEBAPP_URL` в `.env` — бот добавит кнопку меню.

Тесты: `python -m pytest bot/test_bot.py -q` — 37 штук, сеть не трогают. CI дополнительно
собирает тетрадь и проверяет извлекаемый из PDF текст (кириллица и диакритика).
